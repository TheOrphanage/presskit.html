...
const {imagesFolderName, authorizedImageFormats, authorizedVideoFormats} = require('../assets')
...
async function build (dataFilePath, presskit, {
  ...
  if (!config.commands.build.ignoreThumbnails) {
	...
	await createVideoAssets(assetsTarget, presskit.trailers)
  }
...
async function createVideoAssets(target, videos)
{
	for (let video of videos)
	{
		if (video.download === undefined || video.download === null)
		{
			continue;
		}
		
		const basePath = path.join(target, "..")
		const videoPath = path.join(basePath, video.download)
				
		if (fs.existsSync(videoPath))
		{			
			video.isVideoHosted = true;
			...
			try {
				...
				let audioBitrate = 3000
				let videoBitrate = 3000
				
				for (let stream of probeData.streams) {
					switch (stream.codec_type) {
						case 'video':
							videoBitrate = stream.bit_rate;
							break;
						case 'audio':
							videoBitrate = stream.bit_rate;
				...
				const thumbnailPath = path.join(basePath, `${video.download}.thumb.png`)
				...
				const webmPath = path.join(basePath, `${video.download}.webm`)
				console.log('Converting to WebM: ' + webmPath)
				...
				const ogvPath = path.join(basePath, `${video.download}.ogv`)
				console.log('Converting to OGV: ' + ogvPath)
			  ...
			  const msg = chalk.dim(`(${e.message})`)
			  console.warn(`An auto-generated asset for '${chalk.bold(video.download)}' has not been generated. ${msg}`)
			}
		}
	}
}
